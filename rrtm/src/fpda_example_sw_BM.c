...
int main() {
      ...
      cfpda_rrtm_sw (nlay,plev,tlay, h2ovmr, o3vmr, co2vmr, ch4vmr, n2ovmr, o2vmr,
		     &nbands, &band_lbound, &band_ubound, &wgt_sw, &dtau_mol_sw, &dtau_ray_sw);

      for(ib=0;ib<nbands;ib++) {
        tmp += wgt_sw[ib];
        printf("Band %4i :: Wavenumber %8g  %8g [cm^-1] :: Wavelength interval %8g  %8g [nm] :: ck weight %6.3e  \n",ib,band_lbound[ib],band_ubound[ib],1e7/band_ubound[ib],1e7/band_lbound[ib], wgt_sw[ib]);
      }

      // Calculate direct radiation...
      for(ib=0;ib<nbands;ib++) {
        edir[0] = wgt_sw[ib]; // init at TOA with ck weight
        for(k=0; k<nlay; k++) {
          double dtau = dtau_mol_sw[ib][k] + dtau_ray_sw[ib][k];    // direct radiation is subject to extinction due to gas absorption and rayleigh scattering
