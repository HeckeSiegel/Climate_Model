    ...
    double ***dtauray)       // Rayleigh scattering opt depth       [ nbands ] [nlay] 
{
  ...
  static double **dtaumol_save=NULL, **dtauray_save=NULL;
  ...
  if (first) {   /* allocate memory for 2D result fields */
    *dtaumol = (double **)calloc (*nbands, sizeof(double *));
    *dtauray = (double **)calloc (*nbands, sizeof(double *));

    dtaumol_save = *dtaumol;
    dtauray_save = *dtauray;

    first = 0;
  }
  else {
    *dtaumol = dtaumol_save;
    *dtauray = dtauray_save;
  }
