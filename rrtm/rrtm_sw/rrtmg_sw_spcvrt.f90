...
! ------- Modules -------
      ...
      real(kind=rb),allocatable,dimension(:,:) :: fpda_taugas,fpda_tauray
      real(kind=rb),allocatable,dimension(:)   :: fpda_solsrc
...
! Calculate the optical depths for gaseous absorption and Rayleigh scattering
      ...
      if(.not.allocated(fpda_taugas) ) allocate(fpda_taugas(nlayers,ngptsw))
      if(.not.allocated(fpda_tauray) ) allocate(fpda_tauray(nlayers,ngptsw))
      if(.not.allocated(fpda_solsrc) ) allocate(fpda_solsrc(ngptsw))
      fpda_taugas = ztaug
      fpda_tauray = ztaur
      fpda_solsrc = zsflxzen
      return !FPDA
